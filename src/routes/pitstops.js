...
router.get("", (req, res) => {
  ...
  if (driverStandings || constructorStandings) {
    res
      .status(400)
      .send(
        "Bad Request: Pit stop queries do not support standings qualifiers."
      )
      .end();
    return;
  }
  if (circuit || grid || fastest || result || status || constructor) {
    res
      .status(400)
      .send(
        "Bad Request: Pit stop queries do not support the specified qualifiers."
      )
      .end();
    return;
  }
  if (!year || !round) {
    res
      .status(400)
      .send(
        "Bad Request: Pit stop queries require a season and round to be specified."
      )
      .end();
    return;
  }
