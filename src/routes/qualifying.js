...
function formattedQualifyingRow(row) {
  return {
    ...
    Driver: {
      ...
      permanentNumber:
        row.driverNumber != null ? row.driverNumber.toString() : "",
...
function formattedQualifying(rows) {
  ...
  rows.forEach((row) => {
    if (row.year != currentYear || row.round != currentRound) {
      ...
      QualifyingResults[QualifyingResults.length - 1].QualifyingResults.push(
        formattedQualifyingRow(row)
      );
    }
  });
...
router.get("", (req, res) => {
  ...
  if (driverStandings || constructorStandings) {
    res
      .status(400)
      .send(
        "Bad Request: Qualifying queries do not support standings qualifiers."
      )
      .end();
    return;
  }
